...
public class GvrReticlePointer : GvrBasePointer {
  ...
  public override void OnPointerEnter(RaycastResult raycastResultResult, bool isInteractive) {
    ...
    if(isInteractive)
    {
      gazeTime = 1;
      isLooking = true;
      lookingAt = raycastResultResult.gameObject;
      tmp = raycastResultResult;
    }
  }
  ...
  void Update() {
    ...
    if(isLooking && gazeTime <= 0)
    {
      ExecuteEvents.ExecuteHierarchy(lookingAt,
        new PointerEventData(EventSystem.current), ExecuteEvents.pointerClickHandler);

      SliderControler sc = lookingAt.GetComponentInParent<SliderControler>();
      if(sc)
      {
        sc.onClick(tmp);
      }
    ...
    {
      MaterialComp.color = Color.Lerp(Color.white, Color.green, 1 - gazeTime);
    }
